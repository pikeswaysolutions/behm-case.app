  ...
  try {
    ...
    if (path === "excel" && req.method === "POST") {
      ...
      headers.forEach((header: string, index: number) => {
        ...
        if (normalized.includes('case') && normalized.includes('number')) columnMap.case_number = index;
        else if (normalized.includes('sale') && normalized.includes('type')) columnMap.sale_type = index;
        else if (normalized.includes('director')) columnMap.director = index;
        else if (normalized.includes('date') && normalized.includes('death')) columnMap.date_of_death = index;
        else if (normalized.includes('customer') && normalized.includes('first')) columnMap.customer_first_name = index;
        else if (normalized.includes('customer') && (normalized.includes('last') || !normalized.includes('first'))) columnMap.customer_last_name = index;
        else if (normalized.includes('service')) columnMap.service_type = index;
        else if (normalized.includes('total') && normalized.includes('sale')) columnMap.total_sale = index;
        else if (normalized.includes('payment')) columnMap.payments_received = index;
        else if (normalized.includes('date') && normalized.includes('pif')) columnMap.date_paid_in_full = index;
        else if (normalized.includes('ag')) columnMap.aging = index;
      });
